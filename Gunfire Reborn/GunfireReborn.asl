...
init {
	...
	var sceneManagerSig = new SigScanTarget(0, "48 8B 0D ???????? F6 81 ???????? 02 74 ?? 83 B9 ???????? 00 75 ?? E8 ???????? 48 8B 0D ???????? 48 8B 81 ???????? 3A");
	var gameUtilitySig = new SigScanTarget(0, "48 8B 05 ???????? F6 80 ???????? 02 74 ?? 83 B8 ???????? 00 75 ?? 48 8B C8 E8 ???????? 48 8B 05 ???????? 48 8B 80 ???????? 33 D2 89");
	...
	while (!sigsFound) {
		...
		sceneManagerPtr = getPointerFromOpcode(assemblyScanner.Scan(sceneManagerSig), 3, 7);
...
update {
	...
	current.level = vars.memWatchers["level"].Current;
	current.layer = vars.memWatchers["layer"].Current;
